...
export default async function PerformerDetailPage({ params }: PerformerDetailPageProps) {
  ...
  const performer = getPerformer(performerId)
  if (!performer) notFound()

  const Icon = getIcon(performer.icon)
  const services = getServicesByCategory(performer.categoryId).filter(
      ...
      <section className="bg-background py-9">
        <div className="max-w-[900px] mx-auto px-6">
          ...
          <div className="bg-white border border-border rounded-2xl p-8">
            ...
            <div className="mt-6">
              <h2 className="text-sm font-bold text-foreground mb-3">Специализации</h2>
              <div className="flex flex-wrap gap-2">
                {performer.specialties.map((spec) => (
                  <span key={spec} className="text-xs bg-muted text-muted-foreground px-3 py-1.5 rounded-lg font-medium">
                    {spec}
                  </span>
                ))}
              </div>
